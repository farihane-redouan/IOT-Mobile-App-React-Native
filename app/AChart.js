...
function actualiser(setDataX, setDataY){
    const url = "https://api.thingspeak.com/channels/1347195/fields/1.json?api_key=T2I3OY33LCUPH0HH";
    fetch(url).then(function (response){
    return response.json();
}).then(function (obj){
    let len, x,y,y1,y2,DataX = [], DataY = [], Last5;
...
function AChart({title}) {
    ...
    return (
        ...
        <Text style={{fontSize:24, color:'#6cb9e0'}} onPress={actualiser.bind(this,setDataX, setDataY)}>{title}</Text>
        <LineChart
            data={{
                labels: DataX,
                datasets: [
